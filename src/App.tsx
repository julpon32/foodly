...
const unique = (items: string[]) => [...new Set(items.map(x => x.trim().toLocaleLowerCase('ru')).filter(Boolean))]
...
function Icon({ name }: { name: string }) {
  const paths: Record<string,string> = { Home:'m3 10 9-7 9 7v10H3Z M9 20v-7h6v7', Diary:'M5 3h14v18H5Z M9 7h6 M9 11h6 M9 15h4', Insights:'M4 20V12 M12 20V4 M20 20V8', Recipes:'M4 4h6a3 3 0 0 1 3 3v14a4 4 0 0 0-4-2H4Z M13 7a3 3 0 0 1 3-3h5v15h-4a4 4 0 0 0-4 2', profile:'M16 7a4 4 0 1 1-8 0 4 4 0 0 1 8 0 M4 21v-2a8 8 0 0 1 16 0v2', plus:'M12 5v14 M5 12h14' }
  return <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d={paths[name] || paths.plus}/></svg>
}
function App() {
  ...
  function begin(meal?: Meal) {
    ...
    setStep('input'); setIngredient(''); setError(''); setCloseConfirm(false)
  }
  ...
  const cards = (list: Meal[]) => list.map(m => <button className="meal-card" key={m.id} onClick={() => begin(m)}><span className="meal-mark">{m.type === 'Завтрак' ? '☀' : m.type === 'Обед' ? '◒' : m.type === 'Ужин' ? '☾' : '✧'}</span><span className="meal-copy"><small>{m.type} · {m.time}</small><strong>{m.name}</strong><span>{m.ingredients.join(' · ')}</span></span><span aria-hidden="true">›</span></button>)
  return <div className="app">
    <header><a className="brand" href="#" onClick={e => { e.preventDefault(); setTab('Home'); setProfile(false) }}>foodly<span>✳</span></a><button className="avatar" aria-label="Профиль и настройки" onClick={() => setProfile(true)}><Icon name="profile"/></button></header>
    <main>
      ...
      {tab === 'Home' && <><p className="eyebrow">ЕСТЬ РАЗНООБРАЗНО. ЧУВСТВОВАТЬ СЕБЯ ЛУЧШЕ.</p><h1>Хорошее начинается<br/>с маленьких привычек.</h1><p className="intro">Замечайте, что едите.<br/>Открывайте что-то новое каждый день.</p><section className="hero"><div className="botanical" aria-hidden="true">✳</div><span className="pill">Ваш дневник питания</span><h2>{meals.length ? 'Каждый продукт —\nчасть вашей истории.' : 'Первый шаг —\nпросто записать.'}</h2><p>{meals.length ? `${productCount(meals)} разных продуктов в ваших записях. Продолжим?` : 'Без подсчёта калорий и взвешивания. Начните с того, что ели сегодня.'}</p><button className="primary" onClick={() => begin()}><Icon name="plus"/> Добавить еду</button></section><div className="section-heading"><h2>Сегодня</h2><button className="text-button" onClick={() => { setDate(today()); setTab('Diary') }}>Дневник →</button></div>{meals.some(m => m.date === today()) ? cards(meals.filter(m => m.date === today()).sort((a,b) => b.time.localeCompare(a.time))) : <div className="empty-inline"><span>☀</span><p>Здесь появятся ваши приёмы пищи.<br/><small>Можно начать с завтрака или чашки чая.</small></p></div>}<button className="insight-link" onClick={() => setTab('Insights')}><span>✧</span><div><strong>Узнайте свой рацион</strong><small>Ваш обзор и разнообразие продуктов</small></div><span>→</span></button></>}
    ...
    {!draft && <nav aria-label="Основная навигация">{(['Home','Diary','Add','Insights','Recipes'] as const).map(item => item === 'Add' ? <button key={item} className="nav-add" aria-label="Добавить еду" onClick={() => begin()}><Icon name="plus"/></button> : <button key={item} className={!profile && tab === item ? 'active' : ''} aria-current={!profile && tab === item ? 'page' : undefined} onClick={() => { setTab(item); setProfile(false) }}><Icon name={item}/><span>{item}</span></button>)}</nav>}
    {draft && <div className="editor"><div className="editor-inner"><div className="editor-top"><button className="text-button" onClick={() => step === 'confirm' ? setStep('input') : close()}>← {step === 'confirm' ? 'Назад' : 'Закрыть'}</button><span className="brand">foodly<span>✳</span></span></div>{closeConfirm ? <section className="panel"><h2>Закрыть без сохранения?</h2><p>Изменения в этой форме будут потеряны.</p><button className="primary full" onClick={() => setCloseConfirm(false)}>Продолжить запись</button><button className="text-button full" onClick={() => { setDraft(null); setCloseConfirm(false) }}>Не сохранять и закрыть</button></section> : step === 'saved' ? <section className="saved"><span className="success-mark">✓</span><p className="eyebrow">ГОТОВО</p><h1>Ещё один шаг<br/>к разнообразию.</h1><p>Приём пищи сохранён на этом устройстве.</p><section className="panel"><small>{draft.date} · {draft.type}</small><h2>{draft.name}</h2><p>{draft.ingredients.join(' · ')}</p></section><button className="primary full" onClick={() => { setDraft(null); setProfile(false); setTab('Diary') }}>Посмотреть в дневнике →</button><button className="text-button full" onClick={() => { setDraft(null); setProfile(false); setTab('Home') }}>На главную</button></section> : <><p className="eyebrow">{step === 'input' ? 'ШАГ 1 ИЗ 2 · ЗАПИСЬ' : 'ШАГ 2 ИЗ 2 · СОСТАВ'}</p><h1>{step === 'input' ? 'Что вы ели?' : 'Проверим ингредиенты'}</h1>{step === 'input' ? <form onSubmit={e => { e.preventDefault(); if (draft.name.trim()) { setStep('confirm'); setError('') } }}><div className="input-modes"><span className="pill">Текст</span><span>Голос · скоро</span><span>Фото · скоро</span></div><label>Блюдо или описание<textarea autoFocus required maxLength={500} value={draft.name} onChange={e => setDraft({ ...draft, name:e.target.value })} placeholder="Например: тост с авокадо и яйцом"/></label><div className="form-row"><label>Дата<input required type="date" max={today()} value={draft.date} onChange={e => setDraft({ ...draft, date:e.target.value })}/></label><label>Время<input required type="time" value={draft.time} onChange={e => setDraft({ ...draft, time:e.target.value })}/></label></div><fieldset><legend>Приём пищи</legend><div className="type-options">{types.map(t => <button type="button" key={t} aria-pressed={draft.type === t} className={draft.type === t ? 'selected' : ''} onClick={() => setDraft({ ...draft, type:t })}>{t}</button>)}</div></fieldset><p className="muted">На следующем шаге добавьте состав вручную. Граммы и калории не нужны.</p><button className="primary full" type="submit">К ингредиентам →</button></form> : <><p className="intro">{draft.name}</p><p className="muted">Добавьте продукты, из которых состояло блюдо. Пока Foodly не распознаёт состав автоматически.</p><div className="ingredient-list">{draft.ingredients.map((item, i) => <div className="ingredient-row" key={i}><input aria-label={`Ингредиент ${i+1}`} value={item} onChange={e => setDraft({ ...draft, ingredients: draft.ingredients.map((v,n) => n === i ? e.target.value : v) })}/><button aria-label={`Удалить ${item}`} onClick={() => setDraft({ ...draft, ingredients: draft.ingredients.filter((_,n) => n !== i) })}>×</button></div>)}</div><form onSubmit={e => { e.preventDefault(); addIngredient() }}><label>Новый ингредиент<div className="ingredient-row"><input autoFocus value={ingredient} onChange={e => setIngredient(e.target.value)} placeholder="Авокадо, яйцо, хлеб"/><button aria-label="Добавить ингредиент" type="submit">+</button></div></label><p className="muted">Несколько продуктов можно разделить запятой.</p></form><button className="primary full" onClick={save}>Сохранить приём пищи</button></>}{error && <p className="error" role="alert">{error}</p>}{draft.id && <div className="delete-area">{deleteId === draft.id ? <><p>Удалить этот приём пищи из дневника?</p><button className="danger" onClick={() => { if (persist(meals.filter(m => m.id !== draft.id))) { setDraft(null); setDeleteId(null) } }}>Да, удалить</button><button className="text-button" onClick={() => setDeleteId(null)}>Отмена</button></> : <button className="text-button danger" onClick={() => setDeleteId(draft.id!)}>Удалить запись</button>}</div>}</>}</div></div>}
  </div>
}
export default App
